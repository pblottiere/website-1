---
title: "QGIS Server, MapProxy, MViewer et dimension temporelle"
authors: ["Paul BLOTTIERE"]
categories: ["article", "tutoriel"]
date: "2021-08-16 23:23"
description: ""
image: "https://cdn.geotribu.fr/img/articles-blog-rdp/articles/qgis_customize_osgeo4w_rha/qgis_osgeo4w_voiture_rallye.png"
license: default
tags: "QGIS Server,plugin,MapProxy,WMS,TIME,MViewer"
---

# QGIS Server, MapProxy, MViewer et dimension temporelle

:calendar: Date de publication initiale : 6 juillet 2021

**Mots-clés :** QGIS Server | plugin | MapProxy | WMS | TIME | MViewer
...
QGIS Server, bien que timide et silencieux, est un serveur de cartes prospère
et plutôt simple à configurer grâce à QGIS Desktop. Il est aussi très
facilement personnalisable grâce à des plugins Python dédiés et nativement
interfaçable avec des outils standard (serveur de cache, visualisateur web,
...).

Récemment, j'ai découvert le visualisateur Mviewer de GeoBretagne et notamment
sa capacité à intégrer facilement des flux WMS avec la dimension temporelle.
Malheureusement, QGIS Server n'implémente pas (encore) nativement le WMS-T pour
les couches raster. De plus, un serveur de cache comme MapProxy est fondamental
pour fournir des performances décentes, mais le support de la dimension
temporelle n'est pas non plus disponible dans la dernière version...

L'objectif est donc ici de mettre les mains dans le cambouis afin non seulement
de comprendre les rouages sous-jacents mais surtout d'obtenir une chaîne
...
## WMS et dimension temporelle

Le support de la dimension temporelle est disponible dans QGIS Server depuis la
version 3.10 pour les couches vecteur mais n'est pas encore implémenté pour les
couches raster. C'est donc sur ce dernier point que nous allons creuser car il
n'y a actuellement pas de solution clé en main... Mais finalement, ca veut dire
quoi "dimension temporelle"?

Pour une description plus exhaustive, vous pouvez vous référer à l'article
suivant (en anglais): .... mais pour l'instant on peut simplement dire que le
document de référence "OGC Best Practice for using Web Map Services (WMS) with
Time-Dependent or Elevation-Dependent Data (1.0)" spécifie qu'un paramètre TIME
doit permettre de filtrer la donner temporellement.

En d'autres termes, on ne veut pas seulement récupérer une carte sur une zone
géographique précise (via un paramètre BBOX) mais en plus à une date précise.
Il est donc nécessaire d'implémenter un plugin Python pour QGIS Server
permettant d'interpréter ce parmètre TIME pour les couches raster.
